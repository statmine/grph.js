...
function Map() {
  ...
  chart.draw_data = function(data, g) {
    ...
    g.append('rect').attr('width', chart.axes.x.width())
      .attr('height', chart.axes.y.height()).attr('fill', '#F0F0F0');

    g.selectAll("path").data(data).enter().append("path")
      .attr("d", chart.axes.x.transform)
...
function RegionAxis() {
  ...
  // the projection
  var projection_ = d3.geo.projection(function(x, y) { return [x, y];})
  ...
  axis.update_projection = function() {
    if (update_projection_ && map_) {
      projection_ = d3.geo.projection(function(x, y) { return [x, y];})
        .precision(0).scale(1).translate([0, 0]);
      path_ = d3.geo.path().projection(projection_);
      var bounds = path_.bounds(map_);
      var scale  = .95 / Math.max((bounds[1][0] - bounds[0][0]) / canvas_width_, 
                  (bounds[1][1] - bounds[0][1]) / canvas_height_);
      var transl = [(canvas_width_ - scale * (bounds[1][0] + bounds[0][0])) / 2, 
                  (canvas_height_ - scale * (bounds[1][1] + bounds[0][1])) / 2];
  ...
  axis.canvas_height = function(canvas_height) {
    if (!arguments.length || canvas_height == undefined) {
      ...
      update_projection_ = update_projection_ || canvas_height_ != canvas_height;
  ...
  axis.canvas_width = function(canvas_width) {
    if (!arguments.length || canvas_width == undefined) {
      ...
      update_projection_ = update_projection_ || canvas_width_ != canvas_width;
  ...
  axis.domain = function(data) {
    ...
    // all regions are present. If not, we can also zoom in on the selected 
    // regions. 
  ...
  axis.scale = function(region) {
    // Here we should return a path the region.
    if (map_) {
      axis.update_projection();
      return path_(map_.features[index_[region]]);
    }
    return undefined;
  }
...
function ChloroplethAxis() {
  ...
  axis.scale = function(value) {
    ...
    var val    = value - range_[0];
    var val    = Math.sqrt(val*0.9999) / Math.sqrt(range);
    var ncat   = 9;
    var cscale = colorbrewer.GnBu[ncat];
    return cscale[Math.floor(val*ncat)];
  }
