...
function grph_graph_line() {
  ...
  function graph(g) {
    ...
    g.selectAll(".colour").on("mouseover", function(d, i) {
      var variable = axes_.colour.variable();
  ...
  dispatch.on("mouseover", function(variable, value, d) {
    if (variable) {
      var classes = axes_.colour.scale("" + value);
  ...
  dispatch.on("pointover", function(variable, value, d) {
    this.selectAll(".hline").attr("y1", axes_.y.scale(d)).attr("y2", axes_.y.scale(d))
      .style("visibility", "visible");
    this.selectAll(".vline").attr("x1", axes_.x.scale(d)).attr("x2", axes_.x.scale(d))
      .style("visibility", "visible");
  });
